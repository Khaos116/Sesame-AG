...
- `GreenFinance.kt` 使用 `CHANGE_KT1`：单个绿色经营项目出现指定异常后当天仅屏蔽该项目，同时按账号进入可配置的模块冷却时间；冷却结束后继续处理未成功且未屏蔽的项目。
...
2. 绿色经营的真实封控由共享 RPC 入口记录；RPC 层生成的 `9999` 本地屏蔽响应仅跳过当前项目，不得再次触发冷却或中断其他项目。
...
### 11.4 同步步数上限与绿色经营封控冷却

同步步数修复：

- `MyUtils.MIN_SYNC_STEP_COUNT` 与 `MAX_SYNC_STEP_COUNT` 集中定义允许范围 `19853～21423`。
- 配置值改为目标步数下限，随机结果不再与真实步数相加。
- 真实步数已经超过目标上限时保持原值，不尝试降低，也不再额外增加。
- `AntSports.kt` 仅调用 `MyUtils` 的目标生成与合并方法，方便后续定位本地定制。

绿色经营修复：

- 单个 `behaviorCode` 遇到 `1009` 等既有封控响应后，继续沿用稳定 RPC 摘要将该项目屏蔽到当天结束。
- 同时为当前账号写入绿色经营冷却截止时间；账号之间互不影响。
- 默认冷却时间由 `MyUtils.GREEN_FINANCE_COOLDOWN_MINUTES` 控制，当前为 `30` 分钟，可直接修改。
- 冷却期间绿色经营在入口跳过；同一轮刚触发封控时也立即停止剩余绿色经营流程。
- 冷却结束后重新读取服务端项目状态：已成功项目跳过，RPC 层返回 `9999` 的今日屏蔽项目跳过，仅执行尚未成功且未屏蔽的项目。
- 不再使用模块级“当天永久停止绿色经营”，避免一个项目封控影响当天仍可成功的其他项目。
